...
<tr class="border-b dark:border-neutral-500">
    ...
    <td class="whitespace-nowrap px-6 py-4">
      ...
      @elseif($selected == "2")
        <div class="label bg-red text-white">Gastos</div>
      @endif
    </td>
    <td class="whitespace-nowrap px-6 py-4 
      @if($selected == "1") text-green @else text-red @endif">$ {{ $amount }}</td>
    <td class="whitespace-nowrap px-6 py-4">{{ $created }}</td>
    <td class="whitespace-nowrap px-6 py-4">
      ...
      >
        <input type="hidden" name="ref-detail" value="{{ $detail }}" />
        <input type="hidden" name="ref-amount" value={{ $amount }} />
        <input type="hidden" name="ref-selected" value={{ $selected }} />
